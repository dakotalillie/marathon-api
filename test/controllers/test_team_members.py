...
def get_existing_team_members(user_id, team_id):
    return DB.session.query(TEAM_MEMBERS).filter_by(user_id=user_id, team_id=team_id)


def test_team_members_post_without_auth(client):
    """
    WHEN a post request is made to `/team_members` without a token in the `authorization` header
    THEN the response should have a 401 status code and indicate that the header is missing
    """

    response = client.post(
        "/team_members", data=dict(user=str(uuid4()), team=str(uuid4()))
    )
...
def test_team_members_post_with_invalid_auth(client):
    """
    WHEN a post request is made to `/team_members` with an invalid token in the `authorization`
    header
...
def test_team_members_post_missing_parameters(client, test_input, expected):
    """
    WHEN a post request is made to `/team_members` which is missing required fields
...
def test_team_members_post_non_existent_user(client, team1):
    """
    WHEN a post request is made to `/team_members` for a non-existent user
...
def test_team_members_post_non_existent_team(client, user1):
    """
    WHEN a post request is made to `/team_members` for a non-existent team
...
def test_team_members_post_duplicate_member(client, user1, team1):
    """
    GIVEN an existing team and user, with the user being a member of the team
    WHEN a post request is made to `/team_members` for the same team and user
    THEN the response should have a 204 status code and the modification should be ignored
    """

    get_team_members = partial(get_existing_team_members, user1.id, team1.id)
...
def test_team_members_post_success(client, user1, team1):
    """
    GIVEN an existing team and user, with the user not being a member of the team
    WHEN a post request is made to `/team_members` for the given team and user
